...
class ArcballControl {
  ...
  public update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    if (this.isPointerDown) {
      const INTENSITY = 0.2 * timeScale; // Reduced for smoother interaction
      const ANGLE_AMPLIFICATION = 3 / timeScale; // Reduced for more controlled movement
      ...
      const INTENSITY = 0.05 * timeScale; // Smoother deceleration
      quat.slerp(
        this.pointerRotation,
        this.pointerRotation,
        this.IDENTITY_QUAT,
      ...
      if (this.snapTargetDirection) {
        const SNAPPING_INTENSITY = 0.15; // Gentler snapping
    ...
    const RA_INTENSITY = 0.9 * timeScale; // Higher for smoother rotation averaging
    quat.slerp(
      this._combinedQuat,
      this._combinedQuat,
      combinedQuat,
    ...
    const RV_INTENSITY = 0.7 * timeScale; // Smoother velocity transitions
...
class InfiniteGridMenu {
  ...
  private TARGET_FRAME_DURATION = 1000 / 120; // Higher frame rate for smoother animation
  ...
  public run(time = 0): void {
    this._deltaTime = Math.min(16, time - this._time); // Cap at 16ms for smoother frame pacing
    this._time = time;
  ...
  private render(): void {
    ...
    gl.uniformMatrix4fv(
      ...
      false,
      new Float32Array(this.camera.matrices.view)
    );
    gl.uniformMatrix4fv(
      this.discLocations.uProjectionMatrix,
      false,
      new Float32Array(this.camera.matrices.projection)
    );
    gl.uniform3f(
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    let damping = 8 / timeScale; // Increased damping for smoother camera movement
    let cameraTargetZ = 3;
    ...
    if (!this.control.isPointerDown) {
      ...
      cameraTargetZ += this.control.rotationVelocity * 60 + 2.0; // Reduced camera movement sensitivity
      damping = 10 / timeScale; // Increased damping during interaction
    }
